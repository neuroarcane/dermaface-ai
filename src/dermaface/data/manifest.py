...
# --- Label harmonization ----------------------------------------------------
# Substrings that must NOT be mapped even if a keyword below would match — these
# are clear false positives of the keyword rule (e.g. "lupus erythematosus"
# contains "erythema" but is not facial redness). Refine in notebook 02.
EXCLUDE_KEYWORDS: tuple[str, ...] = ("erythematosus", "lupus")
...
LABEL_MAP_COLUMNS = ["source", "raw_label", "mapped_class", "action", "n_rows"]


def build_label_map(
    raw_dir: Path | None = None, out_path: Path | None = None
) -> tuple[Path, int]:
    """Write the raw-condition -> class crosswalk to ``label_map.csv``.

    One row per distinct raw condition string per source, with the class it maps
    to (or ``action='dropped'`` when it maps to none of the four classes). This
    is the auditable record behind the manifest's ``label`` column and the
    starting point for ``notebooks/02_label_harmonization.ipynb``.
    """
    cfg = load_config()
    raw_dir = Path(raw_dir) if raw_dir else (cfg.data_dir / "raw")
    out_path = Path(out_path) if out_path else cfg.label_map_path
    out_path.parent.mkdir(parents=True, exist_ok=True)

    counts: Counter = Counter()  # (source, raw_label) -> n

    # Fitzpatrick17k: the 114-way `label` column.
    fitz = raw_dir / "fitzpatrick17k" / "fitzpatrick17k.csv"
    if not fitz.exists():
        fitz = raw_dir / "fitzpatrick17k.csv"
    if fitz.exists():
        for row in _read_csv(fitz):
            raw = _first(row, "label", "nine_partition_label")
            if raw:
                counts[("fitzpatrick17k", raw)] += 1

    # SCIN: the raw condition actually used for mapping (weighted top, else related_category).
    cases_p = _scin_file(raw_dir, "dataset_scin_cases.csv", "scin_cases.csv")
    labels_p = _scin_file(raw_dir, "dataset_scin_labels.csv", "scin_labels.csv")
    if cases_p is not None:
        labels_by_case = {}
        if labels_p is not None:
            for row in _read_csv(labels_p):
                cid = _first(row, "case_id")
        ...
        for case in _read_csv(cases_p):
            cid = _first(case, "case_id")
            lab = labels_by_case.get(cid, {})
            raw = _parse_weighted_label(_first(lab, "weighted_skin_condition_label"))
            if map_label(raw) is None:
                raw = _first(case, "related_category") or raw
            if raw:
                counts[("scin", raw)] += 1

    rows = []
    for (source, raw), n in counts.items():
        mapped = map_label(raw)
        rows.append(
            {
                "source": source,
                "raw_label": raw,
                "mapped_class": mapped or "",
                "action": "kept" if mapped else "dropped",
                "n_rows": n,
            }
        )
    rows.sort(key=lambda r: (r["source"], r["action"] == "dropped", -r["n_rows"]))

    with out_path.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=LABEL_MAP_COLUMNS)
        writer.writeheader()
...
if __name__ == "__main__":
    ...
    print(f"Wrote {path}")
    print(summ.render())
    lm_path, n = build_label_map()
    print(f"Wrote {lm_path} ({n} distinct raw labels)")
